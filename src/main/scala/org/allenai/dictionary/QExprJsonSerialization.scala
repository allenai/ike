...
object QExprJsonSerialization {
  implicit object QExprFormat extends RootJsonFormat[QExpr] {
    override def write(qexpr: QExpr): JsValue = qexpr match {
      ...
      case q: QGeneralizeTable => q.toJson
    }
  ...
  implicit val qnamedPatternFormat =
    jsonFormat1(QNamedPattern.apply).pack("type" -> "QNamedPattern")
  ...
  implicit val qGeneralizeTable = jsonFormat2(QGeneralizeTable.apply).
    pack("type" -> "QGeneralizeTable")
  implicit val qspFormat = jsonFormat3(QSimilarPhrases.apply).pack("type" -> "QSimilarPhrases")
  implicit val unpackers = Seq(
    qwordFormat,
    qposFormat,
    qchunkFormat,
    qdictFormat,
