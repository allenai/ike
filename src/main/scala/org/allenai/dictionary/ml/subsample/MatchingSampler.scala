...
case class MatchesSampler() extends Sampler() {
  ...
  def limitQueryToDictionary(qexpr: QExpr, table: Table): QExpr = {
    ...
    val limitedQuery =
      if (captureSize == -1) {
        logger.debug(s"Variable length capture group, using full dictionary disjunction of size ${allWords.size}")
        ...
        logger.debug(s"Fixed length capture group, dictionary Disjunction filtered to ${filteredDictionary.size}")
