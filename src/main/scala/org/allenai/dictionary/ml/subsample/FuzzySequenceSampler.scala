...
case class FuzzySequenceSampler(minEdits: Int, maxEdits: Int, captureGroupName: String)
  extends Sampler() {
  ...
  def buildFuzzySequenceQuery(tokenizedQuery: TokenizedQuery, searcher: Searcher): SpanQuery = {
    val asSpanQueries = tokenizedQuery.getSeq().map(q => searcher.createSpanQuery(BlackLabSemantics.blackLabQuery(q)))
    val captureGroup = CaptureSpan(captureGroupName, tokenizedQuery.left.size,
  ...
  override def getLabelledSample(qexpr: QExpr, searcher: Searcher, table: Table): Hits = {
    ...
    val limitingQexpr = QSeq((List.tabulate(precapture)(_ => QWildcard()) :+ QDisj(patternsToUse)) ++ List.tabulate(postcapture)(_ => QWildcard()))
