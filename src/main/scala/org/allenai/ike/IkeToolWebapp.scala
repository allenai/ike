...
object IkeToolWebapp {
  lazy val config = ConfigFactory.load().getConfig("IkeToolWebapp")
  val name = "ike-tool"
  def main(args: Array[String]): Unit = {
    implicit val system = ActorSystem("ike-tool")
    val port = config.getInt("port")
    val service = system.actorOf(Props[IkeToolActor], "webapp-actor")
...
class IkeToolActor extends Actor with HttpService with SprayJsonSupport with Logging {
  import org.allenai.ike.IkeToolWebapp.FutureWithGet
  import org.allenai.ike.JsonSerialization._
