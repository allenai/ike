...
import org.allenai.ike.ml.HitAnalyzer._
import org.allenai.ike.ml.Label._
import org.allenai.ike.ml.compoundop._
import org.allenai.ike.ml.queryop._
import org.allenai.ike.ml.subsample._
...
object QuerySuggester extends Logging {
  ...
  /** Uses a beam search to select the best CompoundQueryOps
    *
    * @param hitAnalysis data to use when evaluating queries
    * @param queryEvaluator evaluator to evaluate CompoundQueryOps with
    * @param opBuilder Builder function to initialize CompoundQueryOps
    ...
    * @return Sequence of CompoundQueryOps and their scores
    */
  def selectOperator(
    ...
    // Counts the number of times each TokenQueryOp occurs in our queue, used to promote diversity
    ...
    priorityQueue.sizeHint(beamSize)
    ...
    // Removes the counts of the operators in op from opsUsed
    ...
    def logQueue(depth: Int): Unit = {
      priorityQueue.clone().dequeueAll.foreach {
        case (op, score) =>
          ...
          logger.debug(s"$opStr\n${queryEvaluator.evaluationMsg(op, depth)}")
      }
    }
    ...
    hitAnalysis.operatorHits.foreach {
      case (operator, matches) =>
        ...
        if (op.isDefined) {
          val score = queryEvaluator.evaluate(op.get, 1)
          addOp(op.get, score)
        }
    }
    ...
    for (i <- 1 until depth) {
      ...
      }.to[Seq].foreach { // iterate over a copy since our queue will be mutated
        case (operator, score) =>
          if (Thread.interrupted()) {
            throw new InterruptedException()
          }
          hitAnalysis.operatorHits.
            // Filter out unusable operators
            ...
            foreach {
              case (newOp, opMatches) =>
                ...
                if (!alreadyFound.contains(newOperator.ops)) {
                  val newScore = queryEvaluator.evaluate(newOperator, 1 + i)
  ...
  def suggestQuery(
    ...
    val tokenizedQuery =
      if (narrow) {
        ...
        val tq = TokenizedQuery.buildWithGeneralizations(
          queryWithNamedCaptures,
          targetTable.cols, similarPhrasesSearcher, searchers, 100
        )
    ...
    // Builds a sequence of Hits to use as our subsample to use when evaluating queries
    ...
    // Preprocess the hits and save the results as a HitAnalysis object
    ...
    val numUnlabelledPhrases = hitAnalysis.examples.filter(_.label == Unlabelled).
      map(_.phraseId).toSet.size
    val expectedUnlabelledPhrases = if (numUnlabelledPhrases == 0 ||
      numDocsSearched == numDocsSearchedForUnlabelled) {
      numUnlabelledPhrases
    } else {
    ...
    val unlabelledBiasCorrection = if (numUnlabelledPhrases == 0) {
      1
    } else {
      Math.min(
        expectedUnlabelledPhrases / numUnlabelledPhrases,
    ...
    val evalFunction =
      if (narrow) {
        SumEvaluator(
          hitAnalysis.examples, config.pWeight, config.nWeight,
          config.uWeight * unlabelledBiasCorrection, true
        )
      } else {
        PartialSumEvaluator(
          hitAnalysis.examples, config.pWeight, config.nWeight,
          config.uWeight * unlabelledBiasCorrection, config.depth, false
        )
      }
    ...
    // Finally score the starting query, and select which suggested CompoundOp to suggest to the
    // user
    val scoredStartingQuery = {
    ...
    // Remove queries the appear to be completely worthless, as marked by Double.NegativeInfinity
    ...
    // Prune queries to make sure we do not return queries that are all very similar to each other
    val operatorsPruneByRepetition = if (narrow) {
      ...
      // Remove the lowest scoring ops that fails our diversity check
      var removed = 0
    ...
    Suggestions(scoredStartingQuery, scoredOps, numDocsSearched)
  }
}
